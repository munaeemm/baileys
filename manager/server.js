...
// ── Config persistence ────────────────────────────────────────────────────────
function loadConfig() {
  if (!fs.existsSync(CONFIG_FILE)) return { supabase_url: '', supabase_key: '', accounts: {} };
...
function pm2Start(name, cwd) {
  try {
    ...
    if (existing) {
      ...
      execSync(`pm2 start index.js --name "${name}" --cwd "${cwd}"`, { encoding: 'utf8' });
    }
...
app.get('/api/config', (req, res) => {
  ...
  res.json({ supabase_url: cfg.supabase_url || '', supabase_key: cfg.supabase_key ? '••••••••' : '' });
});

// POST /api/config
app.post('/api/config', (req, res) => {
  const cfg = loadConfig();
  if (req.body.supabase_url !== undefined) cfg.supabase_url = req.body.supabase_url;
  if (req.body.supabase_key !== undefined && req.body.supabase_key !== '••••••••') cfg.supabase_key = req.body.supabase_key;
  saveConfig(cfg);
  res.json({ ok: true });
});
...
app.post('/api/accounts', (req, res) => {
  ...
  if (!id || !port) return res.status(400).json({ error: 'id and port required' });

  const cfg = loadConfig();
  if (cfg.accounts[id]) return res.status(409).json({ error: 'Account ID already exists' });

  const account = { id, name: name || id, port: parseInt(port), sync_supabase: !!sync_supabase };
...
app.put('/api/accounts/:id', (req, res) => {
  ...
  if (req.body.sync_supabase !== undefined) account.sync_supabase = !!req.body.sync_supabase;
...
app.delete('/api/accounts/:id', (req, res) => {
  ...
  if (req.query.purge === '1') {
    fs.rmSync(path.join(ACCOUNTS_DIR, req.params.id), { recursive: true, force: true });
  }
...
app.post('/api/update-all', (req, res) => {
  ...
  for (const account of accounts) {
    const accountDir = path.join(ACCOUNTS_DIR, account.id);
    if (!fs.existsSync(accountDir)) { results.push({ id: account.id, status: 'folder missing' }); continue; }

    try {
      // Copy only index.js (not .env, not auth/)
      fs.copyFileSync(path.join(TEMPLATE_DIR, 'index.js'), path.join(accountDir, 'index.js'));
      // Optionally update package.json and reinstall if changed
      const templatePkg = fs.readFileSync(path.join(TEMPLATE_DIR, 'package.json'), 'utf8');
      ...
      if (templatePkg !== accountPkg) {
        fs.copyFileSync(path.join(TEMPLATE_DIR, 'package.json'), path.join(accountDir, 'package.json'));
